...
Resolve implementer and validator settings separately under optional [Execution Routing](../coordinator/references/execution-contract.md#execution-routing); preserving authority means following the authorized role policy, not forcing every role onto the orchestrator's settings. Without a policy, retain the existing default. In direct mode, also check any root requirement. Verify requested versus effective routes before affected substantive work, carry resolved settings and any holds into worker assignments, and retain evidence or authorized verification limitations in existing assignment records.
...
In direct mode, apply that rule's intermediate-commentary requirement so a progress reply does not end the active assignment. Preserve the bounded final return at an established assignment stopping boundary.
